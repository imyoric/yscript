...
public class Main {
    ...
    public static String path = "Example.yscr";
    //public static int iset = 0;
    public static void main(String[] args) {

        try {
            //URL url = new URL("https://dzen.ru/");
            //System.out.println();
            if(args.length < 2) {
                if(path == null) {
    ...
    public static void code(BufferedReader reader) throws IOException {
        ...
        while (line != null) {
            ...
            if(code[0].contains("parseOnFile")){
                String filePath = null;
                if(Character.toString(code[1].charAt(0)).equals("'")){
                    filePath = code[1].replaceAll("'", "");
                }else{
                  filePath = Globalvar.get(code[1]);
                }
                String content = null;
                try {
                ...
                if(Character.toString(code[2].charAt(0)).equals("'")){
                    addons.errorMsg("Нельзя вывести текст в текст! Введите переменную!");
                    return;
                }else{
                    Globalvar.put(code[2], content);
                }
                line = reader.readLine();
            }
            if(code[0].contains("parseOnUrl")){
                String url = null;
                if(Character.toString(code[1].charAt(0)).equals("'")){
                    url = code[1].replaceAll("'", "");
                }else{
                    url = Globalvar.get(code[1]);
                }
                String content = null;
                try {
                    ...
                    //e.printStackTrace();
                    addons.errorMsg("Неожиданное исключение! Код 4.");
                    return;
                }
                if(Character.toString(code[2].charAt(0)).equals("'")){
                    addons.errorMsg("Нельзя вывести текст в текст! Введите переменную!");
